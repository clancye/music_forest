...
(function () {
  ...
  // FB#92/#104 (the other half): a reader who had quick unlock on was dropped back
  // to the password screen. The device record lives in IndexedDB, and a *best-effort*
  // origin (the default) can be evicted whole under storage pressure — which on a
  // phone takes the record with it and looks, from the reader's side, like the app
  // forgot them. Turning quick unlock on is exactly the moment they've said this
  // device is theirs, so ask for persistent storage then. Best-effort by design: a
  // denial (or a browser without the API) changes nothing — the passphrase fallback
  ...
  async function save(userId, record) {
    const out = await idb("readwrite", (store) => {
  ...
  window.AOTDDevice = {
    ...
    requestPersistence, UnsupportedError,
  };
})();
