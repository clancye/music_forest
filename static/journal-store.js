...
(function (root, factory) {
   ...
   * asc) ordering. */
  // {uid: choice id} for every record kept — mirrors journal.kept_index(). Album details
  // asks this to say "Kept ✓" for a record kept weeks ago instead of offering to keep it
  // twice (FB#87). Oldest-first so the NEWEST keep of a record wins the key, matching the
  // Python's ORDER BY chosen_at ASC. Pure, like the other analytics: parity-tested.
  function keptIndex(choicesList) {
    const out = {};
    (choicesList || []).slice()
      .sort((a, b) => String(a.chosen_at || "").localeCompare(String(b.chosen_at || "")))
      .forEach((p) => {
        const uid = p.chosen_uid || (p.chosen_id != null ? "d:" + p.chosen_id : null);
        if (uid) out[uid] = p.id != null ? p.id : p._client_id;
      });
    return out;
  }
  ...
  function createStore({ crypto, sync, cache, userId, onChange }) {
    ...
    function _emitChange() { if (typeof onChange === "function") { try { onChange(); } catch (e) {} } }
    // A stable signature of what's currently in memory, to tell whether a background
    // reconcile actually changed anything (→ re-render) vs. matched the cache exactly.
    function _fingerprint() {
      const parts = [];
      for (const k of KINDS) for (const [id, o] of state[k]) parts.push(k + "/" + id + "/" + ((o && o._updated_at) || ""));
      parts.sort();
      return parts.join("|");
    }
    ...
    // FB#92/#104: rows that arrived but wouldn't decrypt under this DEK. Skipping
    // them is right (one bad row must not sink the load), but staying SILENT about
    // them is not: a whole journal encrypted under a different key decrypts to
    // nothing, and "nothing" rendered as "your notebook is empty" is a lie about
    // irreplaceable writing. The count rides out with the feed so the Notebook can
    ...
    // FB#107: sign-out / Delete account wipe the local encrypted cache. Idle/manual
    // LOCK deliberately does NOT — the ciphertext is inert without the dropped DEK,
    // and keeping it is exactly what makes the next unlock instant.
    async function wipeCache() {
      if (_hasCache) {
        try { await cache.clearUser(userId); }
        catch (e) { console.warn("journal cache wipe failed", e && e.message); }
      }
    }
    /** How many stored rows this key couldn't open on the last load. */
    function unreadable() { return _unreadable; }
    ...
    /** Decrypt one ciphertext row into {kind, client_id, obj}, or null if it won't
        ...
        not sink the load). */
    async function _decryptRow(row) {
      try {
    ...
    /** Repopulate state from a set of NON-DELETED ciphertext rows. Decrypts in
        PARALLEL first (each AES-GCM row is independent), then swaps state
        SYNCHRONOUSLY — no await between the clear and the sets — so a read during a
        background reconcile can never catch a momentary empty. Resets _unreadable. */
    async function _repopulate(rows) {
      _unreadable = 0;
      const decrypted = await Promise.all(rows.map(_decryptRow));
    ...
    /** The rows to render from the cache. (Commit B — offline writes — teaches this
        to overlay local pending rows over the synced set; for now the cache holds
        only synced rows, so it's just "drop tombstones".) */
    function _visibleRows(cached) {
      return cached.filter((r) => !r.deleted);
    }

    /** Pull every row and decrypt into memory. Call once after unlock.
     *  FB#107: cache-first. If a local encrypted cache exists, decrypt it and OPEN
     *  the notebook immediately, then reconcile with the server in the background
     *  (offline → keep reading the cache). A cold first open (no cache) blocks on the
     *  server as before — the Notebook shows its loading state meanwhile. */
    async function loadAll() {
      ...
      if (_hasCache) {
        try {
          const cached = await cache.getAll(userId);
          if (cached && cached.length) {
            await _repopulate(_visibleRows(cached));
            _cursor = (await cache.getCursor(userId)) || null;
            _openReady();               // the Notebook can render RIGHT NOW
            cacheHit = true;
          }
        } catch (e) { console.warn("journal cache read failed", e && e.message); }
      }

      if (cacheHit) {
        // Reconcile in the background; an offline failure leaves the cache in place.
        _reconcile(true).catch((e) => console.warn("journal reconcile deferred (offline?)", e && e.message));
        return summary();
      }
      await _reconcile(false);
      return summary();
    }

    /** Reconcile in-memory state + the cache with the server. `background` = we've
        already rendered from the cache, so a pull failure is silent and non-fatal
        (you keep reading offline); a cold pull failure is surfaced (whenReady rejects). */
    async function _reconcile(background) {
      const before = background ? _fingerprint() : null;
      // (a) Replay any pending local writes/deletes so the server catches up FIRST.
      //     Still offline → replay throws; keep the pending rows and stop (background).
      if (_hasCache) {
        try { await _replayPending(); }
        catch (e) {
          console.warn("journal replay deferred (offline?)", e && e.message);
          if (background) return;
        }
      }
      // (b) Full pull from the server.
      let res;
      try {
        res = await sync.getRows({});
      } catch (err) {
        if (background) return;         // already showing the cache — stay offline
        _failReady(err);                // cold, no cache → release parked reads with the failure
        throw err;
      }
      _cursor = res.server_time || null;
      const serverRows = res.rows || [];
      if (_hasCache) {
        // (c) Merge server into the cache, PRESERVING still-pending rows, then rebuild
        //     state from the merged cache (server truth overlaid with local pending).
        try { await _mergeServerIntoCache(serverRows); await cache.setCursor(userId, _cursor); }
        catch (e) { console.warn("journal cache refresh failed", e && e.message); }
        await _repopulate(_visibleRows(await cache.getAll(userId)));
      } else {
        await _repopulate(serverRows.filter((row) => !row.deleted));
      }
      if (!_ready) _openReady();        // cold path: state repopulated, release queued writes
      ...
      if (background && before !== _fingerprint()) _emitChange();
    }

    /** Merge the server's rows into the cache, KEEPING any still-pending local rows
        (a pending write/delete wins over the server copy until it replays). Also
        drops synced cache rows the server no longer has (deleted on another device). */
    async function _mergeServerIntoCache(serverRows) {
      const cached = await cache.getAll(userId);
      const pendingKeys = new Set(cached.filter((r) => r.pending).map((r) => r.kind + "/" + r.client_id));
      const serverKeys = new Set();
      const toPut = [];
      for (const r of serverRows) {
        const key = r.kind + "/" + r.client_id;
        serverKeys.add(key);
        if (pendingKeys.has(key)) continue;          // local pending overrides the server copy
        if (r.deleted) { await cache.removeRow(userId, r.kind, r.client_id); }
        else {
          toPut.push({ kind: r.kind, client_id: r.client_id, ciphertext: r.ciphertext,
            nonce: r.nonce, updated_at: r.updated_at, deleted: false, pending: false });
        }
      }
      if (toPut.length) await cache.putRows(userId, toPut);
      for (const r of cached) {                       // drop synced rows the server dropped
        if (r.pending) continue;                      // never touch a pending local row
        if (!serverKeys.has(r.kind + "/" + r.client_id)) await cache.removeRow(userId, r.kind, r.client_id);
      }
    }
    ...
    async function _putRow(kind, clientId, obj) {
      ...
      const _prev = state[kind].get(clientId);   // for rollback if a no-cache write fails
      state[kind].set(clientId, stored);   // FB#107: in memory immediately (local-first)
      // Try the server; on a network failure, keep the row PENDING in the cache and
      // replay it on the next reconcile / 'online' — the note is never lost. With no
      // cache there's no outbox, so preserve the old contract: a failed write commits
      // NOTHING (roll the optimistic state back) and surfaces the error, so a caller
      // like the guest-buffer migration can retry cleanly with no partial commit.
      let synced = false, serverTime = null;
      try {
        const res = await sync.postRows([{ kind, client_id: clientId, ciphertext, nonce }]);
        synced = true; serverTime = (res && res.server_time) || null;
      } catch (e) {
        if (!_hasCache) {
          if (_prev === undefined) state[kind].delete(clientId); else state[kind].set(clientId, _prev);
          throw e;
        }
        console.warn("journal write deferred (offline?), queued in cache", kind, clientId, e && e.message);
      }
      if (_hasCache) {
        try {
          await cache.putRows(userId, [{ kind, client_id: clientId, ciphertext, nonce,
            updated_at: serverTime || nowIso(), deleted: false, pending: !synced }]);
          if (synced && serverTime) { _cursor = serverTime; await cache.setCursor(userId, _cursor); }
        } catch (e) { console.warn("journal cache write failed", e && e.message); }
      }
      return stored;
    }
    ...
    async function _deleteRow(kind, clientId) {
      ...
      const _prev = state[kind] ? state[kind].get(clientId) : undefined;
      if (state[kind]) state[kind].delete(clientId);   // remove from view immediately
      let synced = false;
      try { await sync.deleteRow(kind, clientId); synced = true; }
      catch (e) {
        if (!_hasCache) {   // no outbox → roll the optimistic removal back, then surface it
          if (_prev !== undefined && state[kind]) state[kind].set(clientId, _prev);
          throw e;
        }
        console.warn("journal delete deferred (offline?), queued in cache", kind, clientId, e && e.message);
      }
      if (_hasCache) {
        try {
          if (synced) {
            await cache.removeRow(userId, kind, clientId);
          } else {
            // Pending delete: a tombstone so a reload doesn't resurrect it; the replay
            // pass issues the server delete when the connection returns.
            await cache.putRows(userId, [{ kind, client_id: clientId, ciphertext: "", nonce: "",
              updated_at: nowIso(), deleted: true, pending: true }]);
          }
        } catch (e) { console.warn("journal cache delete failed", e && e.message); }
      }
    }

    /** Replay every pending (unsynced) local write/delete to the server, oldest-key
        first. On success a row becomes synced (pending cleared / tombstone removed).
        A network failure aborts the pass (throws) — the row stays pending for next
        time. Idempotent: /api/sync writes are, so a re-run is safe. */
    async function _replayPending() {
      if (!_hasCache) return;
      const pending = (await cache.getAll(userId)).filter((r) => r.pending);
      for (const r of pending) {
        if (r.deleted) {
          await sync.deleteRow(r.kind, r.client_id);
          await cache.removeRow(userId, r.kind, r.client_id);
        } else {
          const res = await sync.postRows([{ kind: r.kind, client_id: r.client_id, ciphertext: r.ciphertext, nonce: r.nonce }]);
          const serverTime = (res && res.server_time) || null;
          await cache.putRows(userId, [{ kind: r.kind, client_id: r.client_id, ciphertext: r.ciphertext,
            nonce: r.nonce, updated_at: serverTime || r.updated_at, deleted: false, pending: false }]);
          if (serverTime) { _cursor = serverTime; await cache.setCursor(userId, _cursor); }
        }
      }
    }

    /** Replay pending writes and reconcile with the server. Exposed so the app can
        flush the outbox on a 'online' event (see auth-ui.js). No-op if not ready. */
    async function flush() {
      if (!_dek || !_ready) return;
      try { await _reconcile(true); } catch (e) { /* still offline; try again later */ }
    }
    ...
    // FB#89: a record's own body of writing includes what you wrote about the SONGS
    // on it. Matching `uid` alone excluded them — a track note's uid is
    // `trk:<album>#<pos>`, not the album's — so after FB#106a shipped song tagging you
    // could tag a song and then not find it on the record you tagged it from. A track
    // note carries its parent in `ref.album_uid`, which is what makes this exact rather
    // than a prefix guess. Mirrored in journal.py's for_album so the local single-user
    // build and the hosted one answer the same question.
    function forAlbum(uid) {
      const list = notes().filter((n) =>
          albumKeyOf(n) === uid || (n.ref && n.ref.album_uid === uid))
    ...
    // noteThreads / notesWithTerm were deleted on 2026-08-06 (owner: "there should be
    // NO mention of any threads in the notes"). They read the words you wrote, ranked
    // the ones recurring across your other notes, and returned them as links — which
    // VISION.md P2 rules out directly: what you write is "not mined, not tagged, not
    // built into a picture of you." The record's own story is still a thread you can
    // pull; your writing is not raw material. journal.py's note_threads /
    // notes_with_term went with them.
    ...
    return {
      setKey, locked, ready, whenReady, clear, wipeCache, flush, loadAll, summary, unreadable,
  ...
  const mod = {
    ...
    noteTerms, subjects, subjectGraph, choicesStats, keptIndex, feed, counts,
    journalFeed,
    // store factory
    createStore,
  };
