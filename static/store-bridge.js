 ...
 * replacement is live").
 *
 * app.js still calls the legacy journal routes (/api/journal*, /api/choices,
 * /api/trails, /api/album/<id>/marks). In hosted
...
(function () {
  ...
  async function handle(path, method, init) {
    ...
    // {uid: choice id} for every kept record (FB#87) — the ids ride the same surrogate
    // mapping as the feed's, so "undo" can DELETE /api/choices/<id> with one of them.
    if (path === "/api/choices/kept" && method === "GET") {
      const raw = s.keptIndex();
      const kept = {};
      Object.keys(raw).forEach((uid) => { kept[uid] = sid(raw[uid]); });
      return J({ kept });
    }
  ...
  async function guestHandle(path, method, init) {
    ...
    // A guest keeps too (the cap is on notes, not keeps), so the same "Kept ✓" answer
    // has to come out of the local buffer — oldest-first so the newest keep wins the uid.
    if (path === "/api/choices/kept" && method === "GET") {
      const kept = {};
      if (buf) {
        buf.all().forEach((p) => {
          const uid = p.chosen_uid || (p.chosen_id != null ? "d:" + p.chosen_id : null);
          if (uid) kept[uid] = sid(p.client_id);
        });
      }
      return J({ kept });
    }
    ...
    if (am && method === "GET") {
      ...
      // FB#89: match the signed-in rule — the album's own notes PLUS the notes on its
      // SONGS, which carry their parent in ref.album_uid. `notesForUid` compares the
      // uid exactly, so a guest who tagged a song (FB#106a) found an empty "Your
      // notes" on the very record they tagged it from. Newest first, like every other
      // notes surface, and a `choices` key so the caller's shape is the same either
      // way (a guest's keeps carry no reasons, so it's always empty).
      const all = (buf && buf.notesAll) ? buf.notesAll()
        : ((buf && buf.notesForUid) ? buf.notesForUid(uid) : []);
      const rows = all
        .filter((n) => n.uid === uid || (n.ref && n.ref.album_uid === uid))
  ...
  async function handleJournalFeed(url) {
    ...
    // FB#92/#104: rows the last load couldn't decrypt. A feed of zero notes means
    // two very different things depending on this number, and the Notebook has to
    // be able to tell them apart before it calls anyone's notebook empty.
    if (typeof s.unreadable === "function") feed.unreadable = s.unreadable();
    return J(feed);
  }
  ...
  // handleNoteThreads / handleTermNotes were deleted on 2026-08-06 with the in-note
  // word pull they served. Nothing reads your notes to build links between them any
  // more (VISION.md P2: what you write is "not mined, not tagged").
  ...
  window.fetch = async function (input, init) {
    try {
      ...
      // B35: keep the ONE distinction a person can act on. A sync 401 that even a
      // refresh couldn't rescue means "you're signed out", not "the save broke" —
      // flattening it into a generic 500 is what made an expired session read as
      // lost work. The flag rides the body so callers (which only see the
      // Response, not the Error) can say the true thing.
      if (e && e.sessionExpired) {
        return J({ error: (e && e.message) || "session expired",
