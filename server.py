...
from reqparams import (  # noqa: F401 - imported into the module namespace so server.py's own call sites read unqualified
    _md_or_today,
    _parse_md,
    _platforms_param,
)
from uidmap import (  # noqa: F401 - imported into the module namespace so server.py's own call sites read unqualified
    _album_for_uid,
    _canon_uid,
    _rid_from_uid,
)
...
def _utc_z():
    """Current UTC time as an ISO-8601 'Z' string — the stamp format the admin
    payloads and pushed heartbeats share (lexically sortable)."""
    return datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")


def _backend_name():
    """Which sync store is active: 'postgres' when Supabase is configured, else
    the local 'sqlite' store."""
    return "postgres" if config.SUPABASE_DB_URL else "sqlite"
...
@bp.route("/admin")
def admin():
    """The operator console IS the admin now (2026-07-28): /admin serves the PWA,
    and app.js auto-opens the operator-console overlay on this path (operator-gated
...
# The /api/journal/note/<id>/threads and /api/journal/term routes were removed on
# 2026-08-06 with the in-note word pull they served (VISION.md P2: what you write is
# "not mined, not tagged"). journal.note_threads / notes_with_term went with them.
...
@bp.route("/api/choices/kept")
def api_choices_kept():
    """{uid: choice_id} for every record kept, so Album details can show "Kept ✓"
    instead of offering a second keep (FB#87). Read-only and unenriched — no catalog
    join, unlike the feed below — because it's fetched when a door opens."""
    return jsonify({"kept": journal.kept_index()})
...
@bp.app_errorhandler(auth.AuthError)
...
@bp.app_errorhandler(store.StoreError)
...
def healthz():
    ...
    (for Postgres this also proves the Supabase DB connection); `pool` = the daily
    serving JOIN against pool.sqlite (added 2026-07-24 after a truncated pool.sqlite
    500'd every /api/pool/* while this stayed green). No secrets in the payload —
    only booleans, the backend name, and the app version.

    `pool` is reported but DELIBERATELY NON-FATAL: it never flips the 200/503 that
    Render's healthCheckPath keys on. A corrupt pool on disk survives a restart, so
    a fatal check would loop Render restarting an instance that can't recover — the
    exact crash-loop that locked us out of the Shell on 2026-07-08 — and the rest of
    the app (notebook, sync, legacy /api/today over albums.db) still serves fine
    without the pool. Visibility, not self-destruction: a monitor watching this field
    (or the operator console) catches it in minutes without taking the service down.

    `data` is the MAGNITUDE check (dbsanity), also NON-FATAL for the same reason. The
    catalog/pool/store checks above all ask "does the DB open / answer?" — which a
    valid 2-row test-fixture STUB passes, which is exactly why the 2026-08-09 stub-wipe
    stayed green everywhere while every deck was blank. `data`=false means a served DB
    is implausibly small (a fixture overwrote it); `data_stubs` names which. Fatal here
    would crash-loop Render on a stub (same lockout), so it stays 200 and the loudness
    lives in the operator console banner + the external uptime pinger's alert."""
    checks = {}
    healthy = True
    ...
    try:
        pooldb.health_probe()         # the daily pool serving JOIN must execute
        checks["pool"] = True
    except Exception:                 # noqa: BLE001 - reported, but non-fatal (see docstring)
        checks["pool"] = False

    # Magnitude, not just liveness: is any served DB a stub? Non-fatal (see docstring).
    data_stubs = []
    try:
        data_stubs = dbsanity.stubs()             # pool / catalog / albums below floor
        checks["data"] = (not data_stubs)
    except Exception:                 # noqa: BLE001 - unknown, never fatal
        checks["data"] = None

    # BE2b — COHERENCE, not just magnitude: are the built artifacts the same VINTAGE? A
    # newer pool beside an older catalog/poolsearch, or a rot'd mb_release_map, serves
    # worse SILENTLY (lean cards, shrunken Explore, MB albums vanished from person doors).
    # Non-fatal and degrades to None on 'unknown' — the honesty rule (unknown != a
    # mismatch), exactly like `data`; it never crash-loops Render.
    vint = None
    try:
        vint = vintage.coherence()
        checks["vintage"] = vint["coherent"]      # True / False / None
    except Exception:                 # noqa: BLE001 - unknown, never fatal
        checks["vintage"] = None
    ...
    if data_stubs:
        payload["data_stubs"] = data_stubs        # which DB(s) are stub-sized
    if vint is not None and checks.get("vintage") is False:
        payload["vintage"] = vint                 # which artifact(s) mismatch, + the ids
...
def create_app(overrides=None):
    ...
    # The operator console API (/api/admin/*) lives in its own blueprint module.
    # Imported here (not at module top) so `server` is fully defined before
    # admin_api binds it — the server<->admin_api import cycle stays inert.
    # Gate on the module's PRESENCE, not a bare try/except: the PUBLIC OSS mirror
    # ships without the operator console (docs/oss/SPLIT_PLAN.md) and must run the
    # app unchanged (no admin_api => no /api/admin/*), while a genuine import error
    # *inside* admin_api still surfaces here rather than being swallowed.
    import importlib.util
    if importlib.util.find_spec("admin_api") is not None:
        import admin_api
        application.register_blueprint(admin_api.admin_bp)
