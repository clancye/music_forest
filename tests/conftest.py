...
def _local_mode_env():
    ...
    # Operator gate: the local-mode default is "any authenticated caller is the
    # operator" (an empty set). create_app mutates config in place, so a test that
    # sets OPERATOR_IDS via an override (e.g. test_cost's operator-gate test) would
    # otherwise leak it into later tests sharing the `client` fixture and 403 the
    # admin routes they expect to reach. Reset before + restore after each test.
    config.OPERATOR_IDS = frozenset()
    try:
        yield
    finally:
