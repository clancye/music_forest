...
out = {
  "notes_input": [
     ...
     dict(id=6, uid="per:169094", release_id=None, artist=None, title=None,
          ...
          track=None, timestamp=None)],
  # `id` + `chosen_uid` ride along so keptIndex() has the same material the Python
  # reads out of the table (FB#87) — the ids match the insert order add_choice used.
  "choices_input": [
     dict(id=i + 1, chosen_id=p["chosen"]["release_id"],
          chosen_uid="d:%d" % p["chosen"]["release_id"],
          chosen_artist=p["chosen"]["artist"],
  ...
  "expected": {
    ...
    "kept_index": journal.kept_index(),          # FB#87: uid -> choice id
  }
}
