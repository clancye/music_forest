...
async function main() {
  ...
  // FB#89: forAlbum returns a record's whole body of writing, which now includes the
  // notes on its SONGS — so pick the album note by uid rather than trusting position.
  // (This assertion used notes[0] and started failing the moment track notes joined,
  // which is the test doing its job: the shortcut was only ever valid while a record
  // could have exactly one kind of note.)
  const d100 = storeT.forAlbum("d:100").notes;
  const plainAlbumNote = d100.find((n) => n.uid === "d:100");
  ok(plainAlbumNote && plainAlbumNote.ref == null, "album note carries no ref");
  ok(d100.some((n) => n.uid === "trk:d:100#8"),
    "forAlbum includes notes on this album's tracks (FB#89)");
  ok(d100.every((n) => n.uid === "d:100" || (n.ref && n.ref.album_uid === "d:100")),
    "...and nothing that belongs to another record");
  ok(!storeT.forAlbum("d:999").notes.length,
    "a record with no notes of its own returns none");
