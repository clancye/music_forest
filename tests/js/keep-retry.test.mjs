...
function build(plan) {
  ...
  const code = `
    ...
    let n = 0;
    // A deck that is actually DEALING this record. recordKeep routes a keep into
    // deckState.kept only for a record the deck holds, and into keptOutsideDeck for one
    // kept from Album details outside the deck (FB#87) — so the fixture carries records
    // to exercise the deck path these assertions are about.
    const deckState = { kept: new Map(), records: [{ uid: "d:123" }] };
    const keptOutsideDeck = new Map();
  ...
  `;
  // eslint-disable-next-line no-new-func
  // `remembered` stands in for the durable kept index (FB#87) — recordKeep writes the
  // answer there so a later door can say "Kept ✓" without re-fetching.
  const remembered = new Map();
  const api = new Function(
    "plan", "calls", "met", "markMet", "unmarkMet", "updateSetAsideBar",
    "showToast", "albumKey", "writeNeverReachedApp", "rememberKept", code
  )(
    plan, calls, met,
    ...
    (x) => x && x.uid,
    writeNeverReachedApp,
    (k, id) => remembered.set(k, id)
  );
  return { api, calls, met, remembered };
}
...
{
  const { api, calls, met, remembered } = build(["ok"]);
  ...
  ok(remembered.get("d:123") === 42,
     "and the durable kept index learns the row id, so a later door says Kept ✓");
}
...
// --- keeping a record the deck isn't dealing (FB#87, from Explore) ------------
{
  // The deck holds something else entirely, so this keep belongs to the session map.
  // It must NOT land in deckState.kept: the end-of-deck line reads that map's size to
  // say "You kept N today", and a record kept from Explore was never part of today.
  const { api, calls, met } = build(["ok"]);
  api.deckState.records.length = 0;
  api.deckState.records.push({ uid: "d:999" });
  await api.recordKeep({ uid: "d:123", artist: "A", title: "T" }, "d:123");
  ok(calls.recordChoice === 1, "a keep from outside the deck still writes exactly once");
  ok(met.has("d:123"), "and the record is marked met so today won't re-serve it");
  ok(!api.deckState.kept.has("d:123"),
     "but it never enters deckState.kept — today's kept count stays honest");
}
{
  // The failure path has to give it back from whichever map it went into.
  const { api, calls } = build([502, 502, 502, 502, 502]);
  api.deckState.records.length = 0;
  await api.recordKeep({ uid: "d:123" }, "d:123");
  ok(calls.toasts.length === 1 && /nothing was written/.test(calls.toasts[0].msg),
     "an outside-the-deck failure says what's true — it was never in today's records");
}
