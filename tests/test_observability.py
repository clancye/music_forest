...
def test_healthz_ok_when_deps_up(client, monkeypatch):
    # Pin the pool probe up so this stays hermetic — the fixture provisions albums.db
    # and the store, not a pool.sqlite. The probe's non-fatal semantics on failure are
    # covered by test_healthz_pool_down_is_non_fatal below.
    monkeypatch.setattr(server.pooldb, "health_probe", lambda: 1)
    # The magnitude check's floors are prod-scale (100k rows); the hermetic fixtures are
    # tiny, so pin it healthy here — the stub-detection path is covered by
    # test_healthz_data_stub_is_non_fatal below.
    monkeypatch.setattr(server.dbsanity, "stubs", lambda *a, **k: [])
    # BE2b vintage-coherence check is non-fatal like `data`; pin it coherent here so the
    # test is hermetic (the fixtures aren't vintage-stamped) — the mismatch path is
    # covered by test_healthz_vintage_mismatch_is_non_fatal below.
    monkeypatch.setattr(server.vintage, "coherence",
                        lambda: {"pool_vintage": "v-test", "artifacts": {},
                                 "coherent": True})
    r = client.get("/healthz")
    assert r.status_code == 200
    body = r.get_json()
    assert body["status"] == "ok"
    assert body["checks"] == {"catalog": True, "store": True, "pool": True,
                              "data": True, "vintage": True}
...
def test_healthz_pool_down_is_non_fatal(client, monkeypatch):
    # A truncated/corrupt pool.sqlite must be REPORTED but never 503 the service: a
    # corrupt pool survives a restart, so a fatal check would crash-loop Render (the
    # 2026-07-08 Shell lockout), and the rest of the app still serves without the pool.
    def boom():
        raise RuntimeError("database disk image is malformed")

    monkeypatch.setattr(server.pooldb, "health_probe", boom)
    r = client.get("/healthz")
    assert r.status_code == 200
    body = r.get_json()
    assert body["status"] == "ok"
    assert body["checks"]["pool"] is False
    assert body["checks"]["catalog"] is True


def test_healthz_data_stub_is_non_fatal(client, monkeypatch):
    # A stub-sized served DB (a test fixture overwrote a real one — 2026-08-09) is the
    # false-green this check closes: catalog/pool only test "does it open?", which a
    # 2-row stub passes. It must be REPORTED but never 503 — a fatal check would
    # crash-loop Render on a stub (the same 07-08 lockout). status stays "ok", the loud
    # part is the operator console banner + the uptime pinger's alert.
    monkeypatch.setattr(server.pooldb, "health_probe", lambda: 1)
    monkeypatch.setattr(server.dbsanity, "stubs", lambda *a, **k: ["pool"])
    r = client.get("/healthz")
    assert r.status_code == 200
    body = r.get_json()
    assert body["status"] == "ok"
    assert body["checks"]["data"] is False
    assert body["checks"]["catalog"] is True
    assert body["data_stubs"] == ["pool"]


def test_healthz_vintage_mismatch_is_non_fatal(client, monkeypatch):
    # BE2b: a mixed-vintage artifact set (a new pool beside an older catalog) serves
    # worse SILENTLY. /healthz must REPORT it (checks.vintage=False + the detail) but
    # never 503 — a stale-but-openable catalog survives a restart, so a fatal check would
    # crash-loop Render (the 07-08 lockout). status stays "ok"; the loud part is the
    # operator console + the rsync gate refusing to ship the mixed set.
    monkeypatch.setattr(server.pooldb, "health_probe", lambda: 1)
    monkeypatch.setattr(server.dbsanity, "stubs", lambda *a, **k: [])
    mixed = {"pool_vintage": "vNEW",
             "artifacts": {"catalog": {"pool_vintage": "vOLD", "status": "mismatch"},
                           "poolsearch": {"pool_vintage": "vNEW", "status": "ok"},
                           "mb_release_map": {"pool_vintage": None, "status": "unknown"}},
             "coherent": False}
    monkeypatch.setattr(server.vintage, "coherence", lambda: mixed)
    r = client.get("/healthz")
    assert r.status_code == 200                       # non-fatal — never crash-loops
    body = r.get_json()
    assert body["status"] == "ok"
    assert body["checks"]["vintage"] is False         # reported
    assert body["vintage"]["artifacts"]["catalog"]["status"] == "mismatch"


def test_healthz_vintage_unknown_when_unstamped(client, monkeypatch):
    # An UNSTAMPED pool (pre-BE2b, or before the first rebuild) degrades to unknown, not
    # a mismatch — the honesty rule (unknown != incoherent). checks.vintage is None and
    # NO `vintage` detail is attached (nothing definite to report).
    monkeypatch.setattr(server.pooldb, "health_probe", lambda: 1)
    monkeypatch.setattr(server.dbsanity, "stubs", lambda *a, **k: [])
    monkeypatch.setattr(server.vintage, "coherence",
                        lambda: {"pool_vintage": None, "artifacts": {}, "coherent": None})
    r = client.get("/healthz")
    body = r.get_json()
    assert r.status_code == 200
    assert body["checks"]["vintage"] is None
    assert "vintage" not in body                      # unknown surfaces no false alarm
