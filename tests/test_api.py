...
def test_kept_index(client):
    # FB#87: Album details asks "have I kept this already?" so it can say Kept ✓
    # instead of offering a second keep. Empty to start.
    assert client.get("/api/choices/kept").get_json()["kept"] == {}

    r = client.post("/api/choices", json={"chosen_id": 100})
    cid = r.get_json()["id"]
    kept = client.get("/api/choices/kept").get_json()["kept"]
    assert kept == {"d:100": cid}, "a keep is answerable by uid, with the row's id"

    # Kept twice (a keep, an undo, a keep on a later day): the NEWEST row wins the
    # uid, so an undo removes the one the reader just made.
    second = client.post("/api/choices", json={"chosen_id": 100}).get_json()["id"]
    assert client.get("/api/choices/kept").get_json()["kept"] == {"d:100": second}

    # A record kept over another still answers only for the one you kept.
    client.post("/api/choices", json={"chosen_id": 101, "not_chosen_id": 102})
    kept = client.get("/api/choices/kept").get_json()["kept"]
    assert "d:101" in kept and "d:102" not in kept, \
        "the record you did NOT keep never counts as kept"

    client.delete(f"/api/choices/{second}")
    client.delete(f"/api/choices/{cid}")
    assert "d:100" not in client.get("/api/choices/kept").get_json()["kept"]
