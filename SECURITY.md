...
Part of that private tooling is an **operator console** — a read-only dashboard for
crawler health, catalog quality, and anonymized usage. We keep it out of this repository
because it is operational plumbing, not part of the application you run, and it is not
relevant to auditing the encryption. It is worth being precise about what it can see:
it has no decryption key (the server never holds one), so it cannot read your notebook.
Where it reports on notebook data at all, it counts encrypted rows by kind — never their
contents. The code that *stores* those encrypted rows ([`store.py`](store.py)) and the
code that *encrypts* them ([`static/crypto.js`](static/crypto.js),
[`static/sync.js`](static/sync.js)) are both in this repository, so the boundary the
console operates behind is one you can verify here rather than take on trust.
