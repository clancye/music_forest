...
# BE2a — the LIVE layer: the crawler's irreplaceable WARMTH, split OUT of pool.sqlite
# into its own file so a pool REBUILD (which drop-and-recreates pool.sqlite) can never
# touch it, and a stale-.prev restore of one file can't wipe the other's rows (the
# 08-09 loss: door_links 204,406 -> 125,318). It owns `availability`, `door_links`,
# `crawl_status`, `spotify_daily_log`. pooldb/pooldoor ATTACH it AS `live` via
# poolconn and reference those tables `live.`-qualified; the crawler / prewarms /
# Apple backfill WRITE it; rsync_pool.sh ships it as its OWN leg with its own `.prev`.
# **Absent file => the four tables stay in pool.sqlite exactly as before the split**
# (poolconn.LIVE == ""), so this is a pure no-op until the migration runs — safe to
# ship dark. Migration: tools/migrate_live_split.py (pause crawler -> copy -> verify
# -> resume). See BACKEND_REDESIGN.md §5 (BE2a) and STAGING.md.
LIVE_DB_PATH = Path(os.environ.get("AOTD_LIVE_DB", DATA_DIR / "live.sqlite"))
...
# Data-magnitude ("is this a REAL dataset, or a test-fixture STUB?") floors. A real
# pool/catalog carries ~1.97M rows; a stub is ~2 (incident 2026-08-09: a `pytest
# tools/` run overwrote data/pool.sqlite + data/albums.db with fixtures, the crawler
# rsynced the pool stub to prod, and every deck went blank while /healthz stayed GREEN
# — every check asked "does it open?", which a valid tiny stub passes, none asked "is
# it real-sized?"). Any floor between 2 and ~1.97M cleanly separates them; a rebuild
# always yields ~1.97M, so 100k never false-alarms. Read by dbsanity.py -> the rsync
# ship-gate, /healthz, uptime_ping, and the local db watchdog.
POOL_MIN_ROWS = int(os.environ.get("AOTD_POOL_MIN_ROWS", "100000"))
CATALOG_MIN_ENTITIES = int(os.environ.get("AOTD_CATALOG_MIN_ENTITIES", "100000"))
ALBUMS_MIN_ROWS = int(os.environ.get("AOTD_ALBUMS_MIN_ROWS", "100000"))
# BE2a — the availability floor on live.sqlite (its biggest table: ~1.97M rows, one
# per pool row). A wiped/stubbed live.sqlite would silently zero warmth exactly like a
# stubbed pool once did, so dbsanity floors it too — but only when the file EXISTS and
# is below the floor (a missing live.sqlite reads `unknown`, not a stub, so this never
...
# Every background agent now pushes its own heartbeat (not just the door crawler),
# so the single-slot CRAWL_STATUS_FILE became one file per id under this dir —
# atomic per-id writes are race-free across concurrent pushes. The legacy file is
# still read (and migrated to door.json) so an in-flight door heartbeat isn't lost.
CRAWL_STATUS_DIR = Path(os.environ.get(
    "AOTD_CRAWL_STATUS_DIR", DATA_DIR / "crawl_status"))
...
# drop that line.
#
# RECONCILED 2026-07-27: Render $37.65/mo, Supabase $35.01/mo.
#
# The Render figure is a FORWARD RUN-RATE, deliberately higher than July's projected
# $32.39, because July is not a representative month. The itemized bill shows prod was
# billed 238.19 hr at STARTER ($0.0094/hr) and only 412.46 hr at STANDARD ($0.0336/hr)
# — i.e. ~9.9 days of July predate the H5 Starter→Standard bump on 07-10 and will never
# recur. The disk line tells the same story: 16,567.77 GB-hr over 650.65 hr = a 25.5 GB
# average, against the 30 GB the disk actually is now (grown mid-month). So July's
# projection is diluted by a cheaper past. A steady full month, all-Standard, 30 GB:
#
#   Standard   730 hr × $0.033603/hr            = $24.53
#   Disk       730 hr × 30 GB × $0.0003362/GB·hr = $7.36
#   prod                                          $31.89
#   staging    $5.13 MTD scaled to a full month = $5.76
#   RENDER                                        $37.65
#
# (Both unit rates are DERIVED from cost ÷ usage on the itemized bill — the dashboard
# rounds the disk rate to $0.0003, which under-computes by ~11%. The derived $0.0003362
# × 730 = $0.245/GB/mo, matching Render's published $0.25. Sanity-check any successor
# figure the same way.) CONFIRM against the AUGUST invoice — it is the first full month
# on Standard with the 30 GB disk, so it should land near $37.65 with no Starter line.
#
# Supabase $35.01 is a real invoice figure and covers both projects (prod + staging in
# one org: $25 Pro + ~$10 compute for the second Micro instance).
# The prior defaults (Render $11.75 / Supabase $37.19, reconciled 2026-07-07) had
# gone stale by ~$20 — they predate the Starter→Standard bump (H5, 2026-07-10) — so
# the panel under-reported the nut, and PAID_ACCOUNTS_DESIGN §0 inherited the error.
# Re-reconcile whenever a plan changes; a stale price here quietly flatters the
# break-even math downstream.
#
# Domain + email hosting bill ANNUALLY ($27/yr and $36/yr), amortized to a monthly
# run-rate (÷12). NOTE: the old $0.13 domain default was the $1.54 Porkbun FIRST-YEAR
# SALE price; $27 is the real .lol renewal — ~17× — so this line was wrong by an
# order of magnitude, not a rounding error.
#
# Twilio (A2P 10DLC) bills a recurring monthly campaign fee PLUS the number even at
# zero messages sent, so it belongs here even though SMS sign-in is still on the
# unmerged phase2-passwordless branch. $2.00 is an ESTIMATE — the campaign was
# awaiting approval on 2026-07-27. Replace with the real invoice line.
#
# Total ≈ $75/mo (excludes the LLC's $80/yr annual report, which isn't infrastructure).
COST_RENDER_MONTHLY = float(os.environ.get("AOTD_COST_RENDER", "37.65"))
COST_SUPABASE_MONTHLY = float(os.environ.get("AOTD_COST_SUPABASE", "35.01"))
COST_DOMAIN_MONTHLY = float(os.environ.get("AOTD_COST_DOMAIN", "2.25"))
COST_EMAIL_MONTHLY = float(os.environ.get("AOTD_COST_EMAIL", "3.00"))
COST_TWILIO_MONTHLY = float(os.environ.get("AOTD_COST_TWILIO", "2.00"))
# Plan ceilings for the headroom meters — the caps that would force a bigger bill.
# Render Standard = 2 GB RAM. The disk is 30 GB, NOT render.yaml's `sizeGB: 20` —
# it was grown via the dashboard during the 2026-07-08 disk-full recovery, so the
# yaml is stale and can't be trusted here. 30 GB is MEASURED, not inferred:
# `ssh "$SSH" 'df -h /var/data'` on 2026-07-27 returned 30G total / 16G used / 52%.
# (An earlier revision of this comment derived ~29.6 GB from the Render bill; that
# arithmetic was wrong — the bill covers prod AND staging. Measure, don't derive.)
# Supabase Pro = 8 GB db.
COST_RENDER_DISK_GB = float(os.environ.get("AOTD_COST_RENDER_DISK_GB", "30"))
COST_RENDER_RAM_MB = float(os.environ.get("AOTD_COST_RENDER_RAM_MB", "2048"))
...
# in /api/admin/cost meters against this. Standard runs TWO workers × 8 threads
# (render.yaml `-w 2 --threads 8`) — but `_concurrency` in server.py is a
# per-PROCESS counter, so the gauge only ever sees one worker's in-flight requests.
# 8 is therefore still the correct ceiling. Do NOT "fix" this to 16.
