...
def for_album(uid):
    ...
    with _conn() as c:
        # FB#89: include notes on the SONGS of this album, not just the album row.
        # A track note's uid is 'trk:<album_uid>#<pos>' and carries its parent in the
        # v8 `ref` JSON — matching that is exact, where a LIKE on the uid prefix would
        # also catch an album whose uid is a prefix of another's. Mirrors forAlbum in
        # journal-store.js so local and hosted answer the same question.
        notes = [_note_out(r) for r in c.execute(
            "SELECT * FROM notes WHERE (uid=? OR json_extract(ref,'$.album_uid')=?) "
            "AND deleted_at IS NULL ORDER BY created_at DESC", (uid, uid))]
...
# note_threads / notes_with_term were removed on 2026-08-06 (owner: "there should be
# NO mention of any threads in the notes"). They mined the words you wrote and
# handed back the recurring ones as links between your notes — which VISION.md P2
# rules out directly: what you write is "not mined, not tagged, not built into a
# picture of you." _note_terms stays; `subjects` still uses it.
...
def kept_index():
    """{uid: choice_id} for every record you've kept — the whole history, so Album
    details can say "Kept ✓" for a record you kept weeks ago instead of offering to
    keep it a second time (FB#87). Newest wins when a record was kept more than once,
    so an undo removes the most recent row and any older one stays where it is.

    Deliberately tiny: uid → id only. Everything else about a keep is already
    available from choices_feed(); this answers one question and is cheap enough to
    call on a door opening."""
    with _conn() as c:
        rows = c.execute(
            "SELECT id, chosen_uid FROM choices "
            "WHERE chosen_uid IS NOT NULL ORDER BY chosen_at ASC, id ASC").fetchall()
    return {r["chosen_uid"]: r["id"] for r in rows}
...
def import_data(payload, mode="merge"):
    ...
    with _conn() as c:
        ...
        # Legacy listens (v6+ carry uid; an older export folds onto
        # 'd:'+release_id). Nothing writes to `listens` anymore, but a round-trip
        # must not silently drop history that predates the notes-only journal.
        # Dedup on (uid, listened_at).
        seen_l = set()
        for r in c.execute("SELECT uid, listened_at FROM listens"):
            seen_l.add((r["uid"], r["listened_at"]))
        for lr in listens:
            rid = lr.get("release_id")
            uid = lr.get("uid") or _uid_of(rid)
            at = lr.get("listened_at")
            if uid is None or not at:
                listens_skipped += 1
                continue
            key = (uid, at)
            if key in seen_l:
            ...
            c.execute(
                """INSERT INTO listens
                   (uid, release_id, artist, title, released, discogs_url,
                    listened_at)
                   VALUES (?,?,?,?,?,?,?)""",
                (uid, rid, lr.get("artist"), lr.get("title"),
                 lr.get("released"), lr.get("discogs_url"), at))
            seen_l.add(key)
            listens_added += 1
