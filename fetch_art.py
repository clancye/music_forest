...
# The DOOR resolver's confidence gate (tools/coverage_study._is_match) verifies
# artist AND title; fetch_art historically verified the artist ALONE, which let any
# "Joe Hisaishi" search land a famous Studio Ghibli soundtrack's cover on an
# unrelated record (feedback #82: the 1988 "Piano Stories" wore a "My Neighbor
# Totoro" cover; the live album 14816272 had the Totoro OST art cached). We reuse
# that gate rather than reimplement it, so the art cache and the Listen door can't
# drift apart. Lazy + cached like pooldb._coverage_study: nothing network-facing
# loads until an iTunes lookup runs, and importing fetch_art (fix-art request path)
# stays cheap.
_CS = [None]


def _cover_study():
    """The title gate's normalization/match helpers live in tools/coverage_study —
    part of the private pipeline. In a pipeline-less checkout (the public OSS mirror)
    it's absent; return None so _titles_agree falls back to the artist match instead
    of crashing. Prod ships tools/, so this degradation never triggers there."""
    if _CS[0] is None:
        ...
        try:
            ...
            _CS[0] = False       # sentinel: absent, don't retry the import each call
    return _CS[0] or None


# Least a title must keep AFTER the door gate's normalization for the title check
# to judge it. Non-Latin scripts (CJK, Cyrillic, …) collapse to near-nothing under
# _norm, so below this we can't compare titles and fall back to the artist match —
# preserving the covers the artist gate already trusts for Japanese and other
# non-Latin pressings (a large slice of the Discogs catalogue) rather than dropping
# them. So the title check only ever REJECTS a Latin title that plainly disagrees
# (the wrong-cover class), never removes coverage the artist-only check had.
_TITLE_JUDGEABLE_MIN = 4

# iTunes appends a format tag to the collection name of non-album releases — a
# Discogs "Money" comes back as "Money - Single" / "Money - EP". That's the SAME
# release, but the suffix drops the title ratio under the gate's floor and would
# reject a correct single/EP cover (measured: ~half of a 60-cover audit sample were
# these false rejections). Strip it on both sides before comparing; _norm already
# handles parenthetical "(Remastered)" / "(Deluxe)" tags.
_RELEASE_SUFFIX = re.compile(r"\s*-\s*(single|ep)\s*$", re.I)


def _titles_agree(artist, title, got_artist, got_title):
    """True unless BOTH titles survive normalization (are judgeable) AND the shared
    door gate says they're different albums. A trivial/non-Latin title defers to the
    artist match; iTunes' "- Single"/"- EP" format tag is stripped so it never counts
    as a difference."""
    cs = _cover_study()
    if cs is None:
        return True              # no cover-study module (pipeline-less) — defer to the artist match
    wt = _RELEASE_SUFFIX.sub("", title or "")
    gt = _RELEASE_SUFFIX.sub("", got_title or "")
    if (len(cs._norm(wt)) < _TITLE_JUDGEABLE_MIN
            or len(cs._norm(gt)) < _TITLE_JUDGEABLE_MIN):
        return True
    return cs._is_match(artist, wt, got_artist, gt)
...
def itunes_lookup(artist, title):
    ...
    whose artist AND title match, or (None, None, None) if none do.

    We ask for several results and verify BOTH fields (B5, feedback #82) instead of
    trusting iTunes' top hit: a generic title like "What's Going On" otherwise
    returns Marvin Gaye's cover no matter whose album we're looking up, and — the
    #82 case — any "Joe Hisaishi" search returns a famous Ghibli soundtrack's cover.
    The title check defers to the artist match for non-Latin titles it can't judge
    (see _titles_agree). Returning nothing on a miss is intentional — a placeholder
    beats a confidently-wrong cover."""
    results = itunes_search(f"{_norm(artist)} {_norm(title)}",
                            limit=ITUNES_MATCH_LIMIT)
    for hit in results:
        got = hit.get("artistName") or ""
        if _artist_matches(artist, got) and _titles_agree(
                artist, title, got, hit.get("collectionName") or ""):
